# PyHC Repo Template

[![PyHC Community Package badge](https://img.shields.io/badge/PyHC-Community%20Package-brightgreen?logo=data:image/png;base64,)](https://heliopython.org/projects)
[![DOI](https://zenodo.org/badge/954874530.svg)](https://doi.org/10.5281/zenodo.15191528)

## Overview

This repository template provides a solid foundation for new Python packages in the Heliophysics Community (PyHC). It follows all PyHC standards and best practices, making it easy for developers to start new projects that will integrate well with the PyHC ecosystem.

By using this template, you'll get:
- A standardized project structure
- Testing setup with pytest
- Documentation scaffolding
- Style guide adherence with flake8/PEP 8
- Automated PyPI publishing workflow
- Citation file setup
- License and code of conduct templates

## How to Use This Template

1. Click the green "Use this template" button at the top of this GitHub repository
2. Name your new repository and provide a description
3. Clone your new repository locally
4. Update the `pyproject.toml` file with your package information
5. Rename the `src/my_pyhc_package` directory to match your package name
6. Start developing your awesome heliophysics package!

## Code Style

All PyHC projects must follow the PEP 8 style recommendations. This template has been pre-configured to comply with these standards.

### Style Guidelines:
- Follow PEP 8 conventions
- Use static analysis tools to identify style deviations:
  - flake8
  - pylint
  - Ruff (recommended)

To check your code style:
```bash
flake8 src/ tests/
```

## Testing

Testing is essential for PyHC packages. This template includes a basic test setup using pytest.

### Testing Guidelines:
- Write unit tests for individual components (functions, classes)
- Write integration tests that verify interactions between components
- Measure test coverage

To run tests:
```bash
# Run all tests
pytest tests/

# Run a single test
pytest tests/test_main.py::test_hello_world

# Run tests with coverage
pytest --cov=src tests/
```

## Publishing Your Package

### Publishing to PyPI

This template includes a GitHub Actions workflow that automatically publishes your package to PyPI when you create a new release on GitHub.

1. Ensure your `pyproject.toml` is properly configured
2. Create a PyPI API token and add it to your repository secrets as `PYPI_API_TOKEN`
3. Create a new release on GitHub
4. The workflow will automatically build and publish your package

For more details, see the [PyPI documentation](https://packaging.python.org/en/latest/tutorials/packaging-projects/).

### Publishing to conda

To make your package available via conda:

1. Create a conda recipe (usually a `meta.yaml` file)
2. Submit your package to conda-forge

For detailed instructions, see the [conda-forge documentation](https://conda-forge.org/docs/maintainer/adding_pkgs.html).

## DOI Integration with Zenodo

To automatically mint DOIs for your releases:

1. Go to [Zenodo](https://zenodo.org/)
2. Log in with your GitHub account
3. Navigate to your GitHub account settings in Zenodo
4. Enable the repository you want to track
5. Create a new release on GitHub
6. Zenodo will automatically mint a DOI for your release

## Citation File

This template includes a CITATION.cff file that follows the [Citation File Format](https://citation-file-format.github.io/) specification. Update this file with your project's information to make it easy for others to cite your work.

For more information, see GitHub's [documentation on citation files](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-citation-files).

## PyHC Environment

The PyHC Environment is a comprehensive Python environment that includes all published PyHC packages. When developing your package, it's important to ensure compatibility with this environment.

### Dependency Management:

- Follow [PHEP 3](https://github.com/heliophysicsPy/standards/pull/29) guidelines for dependency management
- Avoid dependencies that would conflict with existing PyHC packages
- To check for potential conflicts, refer to the [PyHC Environment's dependency spreadsheet](https://github.com/heliophysicsPy/pyhc-docker-environment/tree/main/spreadsheets)

## Documentation

It's recommended to use [Read the Docs](https://readthedocs.org/) for hosting your project documentation. This template includes a basic `docs/` directory where you can build your documentation.

For Python documentation, consider using:
- Sphinx
- MkDocs
- Jupyter Book

## Code of Conduct

This template includes a Code of Conduct based on the [Contributor Covenant](https://www.contributor-covenant.org/). Please review and modify `CODE_OF_CONDUCT.md` as needed for your project, but ensure it remains compatible with the Contributor Covenant.

## License

This template includes a permissive license. PyHC recommends using permissive open-source licenses such as:
- BSD 2-clause
- BSD 3-clause
- BSD+Patent

Copyleft licenses like GPL are not recommended for PyHC projects.

You may choose a different license for your project by updating the `LICENSE` file, but make sure to select an OSI-approved permissive license.

## PyHC Standards

For the complete set of PyHC standards that this template follows, please refer to the [PyHC Standards Document](https://github.com/heliophysicsPy/standards/blob/main/standards.md).